...
workflow {
   ...
   // Checks genome only is in S3, else if its a path, makes sure its an absolute path. 
   genomeonly.map{ name, fasta -> 
      def full_fasta = fasta =~ /^s3:\/\// ? fasta : new File(fasta).getAbsolutePath()
      [name, full_fasta]
   }.set { genomeonly_fullpath } 

   //genomeonly_fullpath.view()
   ...
   if (params.earlgrey){
      EARLGREY (GFFREAD.out.just_genome.mix(genomeonly_fullpath))
   }

   if (params.hite){
      HITE (GFFREAD.out.just_genome.mix(genomeonly_fullpath))
   }
